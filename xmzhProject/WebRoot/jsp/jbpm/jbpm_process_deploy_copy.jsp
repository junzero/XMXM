...
<h:form name="data_form"  id="data_form" action="/jbpm/jbpmDemoAction_deployUptProcess.action"  enctype="multipart/form-data" method="post" >
...
<input type="hidden" id="pid" name="processDeployDto.id" value="<b:write property="processDeployDto.id"/>">
			...
			<tr>
		 	...
		 	<tr>
				...
				<d:select
						...
						name="processDeployDto.businessType"  property="processDeployDto.businessType" value="" />
			...
			<tr>
					...
					<td colspan="1"  width="30%">
					<h:select style="width:148px;" id="processState" name="processDeployDto.processState"  validateAttr="allowNull=false;" >
      	...
      	<!-- <tr>
					...
					<td colspan="1"  width="30%">
				<input type="file"  id="upload" name="upload" style="width:50%" validateAttr="allowNull=false;" />
					</td>
					</tr> -->
					<tr>
				...
				try {
					if (data.indexOf("success") >= 0) {
							/* if (confirm('复制流程成功! 是否继续节点配置?')){
								...
								unMaskTop();
								window.parent.location.href = encodeURI(strUrl);	
								window.close();
							}else{
								unMaskTop();
							...
							var hh=data.split(",");
							var strUrl = "/jbpm/jbpmDemoAction_toProcessImgConfig.action?taskAssgineeDto.definitionId="+hh[1];
							unMaskTop();
							window.parent.location.href = encodeURI(strUrl);	
							window.close();
