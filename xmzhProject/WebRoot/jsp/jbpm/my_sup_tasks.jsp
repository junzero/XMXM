...
<html>
	<head>
		...
		<script>
			function clears(){
					$("#preTaskAssingeeName").val("");
					$("#preTaskOrgName").val("");
					$("#preTaskTimeStart").val("");
					$("#preTaskTimeEnd").val("");
	...
	<h:form name="appQuery"	action="/jbpm/tProcessTaskAssigneeAction_queryMySupTasksList.action" method="post">
		<w:panel id="panel1" title="督办信息">
			<table align="center" border="0" width="100%" class="form_table">
				<tr>
					<td class="form_label" align="right" width="20%">提交人姓名：</td>
					<td colspan="1"  width="30%"><h:text id="preTaskAssingeeName" property="taskAssignee.preTaskAssingeeName" /></td>
					<td class="form_label" align="right" width="20%">提交机构名称：</td>
					<td colspan="1"  width="30%"><h:text id="preTaskOrgName" property="taskAssignee.preTaskOrgName" /></td>
					</tr>
					<tr>
					<td class="form_label" align="right" width="20%">提交时间：</td>
					<td colspan="3">
					从<w:date format="yyyy-MM-dd"submitFormat="yyyyMMdd" name="preTaskTimeStart" property="taskAssignee.preTaskTimeStart" />
					到<w:date format="yyyy-MM-dd"submitFormat="yyyyMMdd" name="preTaskTimeEnd" property="taskAssignee.preTaskTimeEnd" />
					</td>
				</tr>
	...
	<DIV class="divList">
			<w:panel id="panel" width="100%" title="查询结果">
				...
				<h:form name="page_form"
					action="/jbpm/tProcessTaskAssigneeAction_queryMySupTasksList.action" method="post">
		...
		function handleTask(){
			var gop = $id("group1");
	  		...
	  		if(len == 0){
				...
				* 2014.9.1 改为弹出框
				**/
		  		showModalCenter(strUrl, null, callBack, clientX*0.8, clientY*0.65, '待办详情');
			}
		}
		function callBack(){
			  location.reload();
