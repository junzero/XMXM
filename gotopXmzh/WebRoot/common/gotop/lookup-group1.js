	function lookUp_selectFunc(row){
		if(row.getStatus()){}else{
			...
			if(selObj){
				...
				oOption.selType=row.group.entityName;
				var showText = posiname;
			...
			var checkcount = row.group.checkcount;
			if(checkcount){
				if(checkcount<selObj.options.length){
					window.parent.deleteOption(selObj,0);
				}
			}
		}
	}
	...
	CheckGroup.prototype.initLookUp_CheckGroup11 = function(butID){
...
//			this.selectBox.options.length=0;
		...
		if(dialogArguments && dialogArguments['Role'] && dialogArguments['Role'].length>1){}else{
		...
		var selObj = window.parent.$id(mngemps.relatedSelID);
		if(isBlank(selObj)){
			return;
		}
		...
		if(args.toString()){
			...
			mngemps.entityProperty = this.checkgroup.submitParamName;
			mngemps.displayOtherProperty = this.checkgroup.displayParamName;
		}
		var sRows = this.getRows();
		for(var i=0;i<arraEntity.length;i++){
			var entity = arraEntity[i];
			var pstr = entity.getProperty(mngemps.entityProperty);
			if(isBlank(pstr)){
				continue;
			}
			oOption = document.createElement("OPTION");
			...
			selObj.options.add(oOption);
			pstr = entity.getProperty(mngemps.displayOtherProperty)
			if(oOption.selType == "Role"){
				pstr += "（角色）";
			}
			...
			for(var j=0;j<sRows.length;j++){
				var row = sRows[j];
				if(!row.getStatus()){
					var positionid = row.getParam(mngemps.entityProperty);
					if(positionid==pstr){
	...
	CheckGroup.prototype.initLookUp_CheckGroup = function(butID){
		var mngemps = window.parent.$id(butID);
		this.inited = true;
		var options = this.selectBoxID.options;
		var sRows = this.getRows();
		this.selectBox = this.selectBoxID;
		for(var i=0;i<options.length;i++){
			if(options[i].selType==this.entityName){
				var pstr = options[i].value;
				for(var j=0;j<sRows.length;j++){
					var row = sRows[j];
					if(!row.getStatus()){
						var positionid = row.getParam(mngemps.entityProperty);
	...
	CheckGroup.prototype.initLookUp_Datacell = function(mngemps){
		if(this.inited!=true){
			this.initLookUp_CheckGroup(mngemps);
			return;
		}	
