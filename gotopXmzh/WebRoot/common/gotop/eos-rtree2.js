...
function relatedSelByNode(parentNode,node,entity,isFirstNode,isLastNode,hasRoot,iconsrc,iconType,name){
	...
	if(selID){
		...
		}else if(oOption && $id(selID)){
			$id(selID).removeChild(oOption);
			node.optionSel=null;
		}
		var checkcount = Infol.infoObj.checkcount;
		if(checkcount){
			if(checkcount<$id(selID).options.length){
				$id(selID).options[0].treeNode.changeCheckboxState();
				$id(selID).options[0]=null;
			}
		}
//		if($id(selID).options.length 
	}
}
...
function _rtreemodel_expandAllFun(node){
	node.expandNode();
	_rtreemodel_loadFinishedFun(node);
}
function _rtreemodel_loadFinishedFun(node){
	if(node.isExpanded()){
		var children = node.getChildren();
    	for(var i=0;i<children.length;i++){
    		var tempNode = children[i];
    		if(tempNode.entity.name=='Tomorganization'){
    			if(tempNode.isExpanded()){}else{
	    			tempNode.expandNode();
    			}
	    		_rtreemodel_loadFinishedFun(tempNode); 
    		}
    	}
	}else{
		window.setTimeout(_rtreemodel_loadFinishedFun,1,node);
	}
}