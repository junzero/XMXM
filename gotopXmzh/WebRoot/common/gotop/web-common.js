	...
	*   cheValue:欲设置的数组
	*/
	function setCheckValue(radioName,cheValueArra){
		var radioObj;
		if(typeof(radioName)=='string'){
			radioObj = document.getElementsByName(radioName);
		}else{
		...
		for(var i=0;i<radioObj.length;i++){
			for(var j=0;j<cheValueArra.length;j++){
				var cheValue = cheValueArra[j];
				if(radioObj[i].value==cheValue){
					radioObj[i].checked="checked";
					break;
				}else{
...
*/
	...
	*/
    function paramFunc(comboSobj){
    	var filterModel = comboSobj.filterModel;
...
**/
	...
	ParamEntity.prototype.setProperty = function(key,values) {
		if(values){
		    ...
		    for(var i=0;i<valueArra.length;i++){
		    	...
		    	if(isBlank(entity)){
					entity = this.arraEntity[i] = new Entity(this.id);	
		    	}
	    		entity.setProperty(key,valueArra[i]);
		    }
		    entity.tagName=this.id;
		    this[key]=values;
	    }
	};
	...
	**/
	function organ_demo(){
		//<tr onclick="clicktrfun(this)"
	  	var tempTRr = null;
	  	var newTR = null;
	  	var table = $id("ta");
	  	function clicktrfun(objt){
			if(newTR!=null){
		  		//删除展开
		  		if(newTR==null){
					return;
		  		}
		  		table.deleteRow(newTR.rowIndex);
		  		newTR=null;
			...
			//创建展开tab
			tempTRr=objt;
	  		newTR = table.insertRow(objt.rowIndex+1);
	  		var newNameTD=newTR.insertCell(0);
	  		
	  		newNameTD.colSpan=objt.cells.length;
	  		newNameTD.innerHTML='<iframe id="tab" style="width:100%;height:200" frameBorder="0" scrolling="auto" src="/eos-default/xtwh/bkflgl/DataCell.jsp"></iframe>';
	  	}
	}
	/**
	*	提交前逐个检查每个单元格，验证不正确的单元格，并使之进入编辑状态
	**/
  	function datacell_validate_demo(){
  		var datacell = $id("cell2");//获得相应datacell
  		var alltr = datacell.getAllRows(true);
  		for(var i=0;i<alltr.length;i++){
  			var B = datacell.getEntity(alltr[i]);
  			for(var j=0;j<alltr[i].cells.length;j++){
  				var tdobj = alltr[i].cells[j];
  				if(tdobj.innerText){}else{  //对比的条件
  					datacell.setActiveCell(tdobj);
  					if (B.status != Entity.STATUS_NEW){
	  					B.status = Entity.STATUS_MODIFIED;
  					}
  					datacell.startEdit();//f_alert    checkInput
  					return;
  				}
  			}
  		}
    ...
    * orgtempseq最终节点
    **/
    function findLoginOrg(node,orgtempseq){
    	if(node){}else{
    		return;
    	}
    	if(orgtempseq){}else{
    	...
    	if(!node.isExpanded()){
	    	window.setTimeout(findLoginOrg,100,node,orgtempseq);
			return;
    	}
    	for(var i=0;i<children.length;i++){
    		...
    		if(orgtempseq==orgseq){
		    	...
		    	tempNode.expandNode();
				window.setTimeout(findLoginOrg,100,tempNode,orgtempseq);
				return;
			}
    	}
    	node.select();
    ...
    /**
    *  get CellIndex by fieldName
    *  prarm dataid
    */
   	function getCellIndex(dataid,fieldName){
   		var fields = $id(dataid).fields;
   		var i=0;
