...
//根据节点的状态,设置节点的图标
	...
	var pcheck = undefined;
	if(rtreeNode.entity.checked){
		var checkType = findRTree(rtreeNode).model.checkButton;
		if(checkType){
			if($id(checkType)){
				checkType = $id(checkType).checkType;
				if(checkType=="associated"){
					pcheck = 1;
				}
			}
		}
