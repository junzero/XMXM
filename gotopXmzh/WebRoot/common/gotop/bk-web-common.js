...
*/
    ...
    * 使页面上的对象Enter执行TAB,参数为范围对象的name/ID
    */
    function tabEnterjs(name,eventKey){
    	var ebnT = null;
		if(name && typeof(name) == "string"){
			nameObj = $id(name) || $name(name);
		}else{
		...
		if(eventKey){
		    eventManager.add(nameObj, eventKey,
		    function(A) {//避免与datacell冲突
		...
		ebnT = nameObj.getElementsByTagName("select");
		for(var i=0;i<ebnT.length;i++){
			ebnT[i].onkeydown=function(){
				if((event.keyCode>=48 && event.keyCode<=57) || (event.keyCode>=65 && event.keyCode<=90) || (event.keyCode>=96 && event.keyCode<=105)){
					var keyvalue = String.fromCharCode(event.keyCode);
		...
		ebnT = nameObj.getElementsByTagName("a");
		for(var i=0;i<ebnT.length;i++){
			if(ebnT[i].tabIndex){}else{
				ebnT[i].tabIndex=-1;		
			}
		}
    	return true;
    ...
    function replaceOnblur(obj){
		...
		obj.onblur=function(){
			...
			var okd = null;
			try{
   				this.style.backgroundColor=this.old_BackgroundColor;
//   				this.style.backgroundColor="";
    ...
    function replaceOnFocus(obj){
		...
		obj.onfocus=function(){
			if((''+obj.keyOnblurTpye) != 'null'){
				obj.keyOnblurTpye=null;
			}else{
				obj.keyOnblurTpye=false;
			}
			obj.keyOnchangeTpye=null;
			obj.state="start";
			obj.defaultValue_focus = this.value;
			if(obj.oldOnfocus){
				okd = obj.oldOnfocus();
   				if(okd==false){
   					return false;
    ...
    */
    function replaceOnchange(obj){
		...
		obj.onchange=function(){
			...
			var okd = null;
			try{
    			if(this.defaultValue_focus != this.value && this.oldOnchange){
	...
	*/
	...
	function keyCode9(obj){
		...
		if(obj.beforeEnterEvent && eval(obj.beforeEnterEvent)==false){
			if(obj.afterEnterEvent){eval(obj.afterEnterEvent)}
			return false;
		}
		...
		if(obj.afterEnterEvent && eval(obj.afterEnterEvent)==false){
			return false;	
		}
	}
...
*/
	...
	*/
    ...
    function paramFunc(comboSobj){
	    ...
	    if(comboSobj.id=="vSplsh"){//加入年份vSplsh
	    var cnfws="";
		    if($id("cNf")!=null){
		    cnfws= $id("cNf").value; 
		    }else{
	    ...
	    comboSobj.addParam("nf",cnfws);
	    
		    if($id("pub_isdd")!=null){//是否征订单--订单
		    var pub_isdd=$id("pub_isdd").value;
		    comboSobj.addParam("pub_isdd",pub_isdd);
		    }
		    if($id("pub_zysz")!=null){//是否增要设置
		    var pub_zysz=$id("pub_zysz").value;
		    comboSobj.addParam("pub_zysz",pub_zysz);
		    }
		    if($id("pub_clybz")!=null){//来源标志
		    var pub_clybz=$id("pub_clybz").value;
		    comboSobj.addParam("pub_clybz",pub_clybz);
		    }
		    if($id("pub_dhjgid")!=null){//订货机构ID
		    var pub_dhjgid=$id("pub_dhjgid").value;
		    comboSobj.addParam("pub_dhjgid",pub_dhjgid);
		    }
		    if($id("pub_zddh")!=null){//订货机构ID征订单号
		    var pub_zddh=$id("pub_zddh").value;
		    comboSobj.addParam("pub_zddh",pub_zddh);
		    }
		     if($id("pub_wfbk")!=null){//外发报刊  pub_wsh
		    var pub_wfbk=$id("pub_wfbk").value;
		    comboSobj.addParam("pub_wfbk",pub_wfbk);
		    }
		       if($id("pub_wsh")!=null){//包涵未审核报刊 
		    var pub_wsh=$id("pub_wsh").value;
		    comboSobj.addParam("pub_wsh",pub_wsh);
		    }
		      if($id("pub_kq")!=null){//是要关联刊期 
		    var pub_kq=$id("pub_kq").value;
		    comboSobj.addParam("pub_kq",pub_kq);
		    }
	    
	    }
	    if(comboSobj.id=="cpid"){//加入年份cpNf
	     var cnfws="";
		    if($id("cpNf")!=null){
		    cnfws= $id("cpNf").value; 
		    }else{
	    ...
	    comboSobj.addParam("nf",cnfws);
	      ...
	      if($id("pub_cps")!=null){//包涵未审核报刊 
		  var pub_cps  = $id("pub_cps").value; 
		  comboSobj.addParam("pub_cps",pub_cps);
		  
		    }
		    if($id("pub_sh")!=null){//包涵未审核报刊 
		  var pub_sh  = $id("pub_sh").value; 
		  comboSobj.addParam("pub_sh",pub_sh);
		  
		    }
		    
	    
	    }
	      if(comboSobj.id=="othercpid"){//加入年份cpNfothercpid
	     var cnfws="";
		    if($id("cpNf")!=null){
		    cnfws= $id("cpNf").value; 
		    }else{
	    ...
	    if(comboSobj.id=="vSplsh1"){//加入年份vSplsh1
	    ...
	    if(comboSobj.id=="jg"){
	       if($id("jgjb") != null){
		      var jgjb= $id("jgjb").value;
		      comboSobj.addParam("jgjb",jgjb);
		   }
	    }
	    ...
	    if(comboSobj.id=="fxs"){
	       ...
	       if($id("cXz") != null){
		      var cXz= $id("cXz").value;
		      comboSobj.addParam("cXz",cXz);
		   }
		   if($id("vQydmlsh") != null){
			   if($id("vQydmlsh").getSelectEntity() != null &&  $id("vQydmlsh").getSelectEntity().getProperty("vQydmlsh") != null && $id("vQydmlsh").getSelectEntity().getProperty("vQydmlsh") != ""){
				      var vQydmlsh= $id("vQydmlsh").getValue();
				      comboSobj.addParam("vQydmlsh",vQydmlsh);
			   }
		   }
	    }
	    ...
	    if(comboSobj.id=="gys"){
	       if($id("allgysflag") != null){
		      comboSobj.addParam("allgysflag","1");
		   }
	    }
	    
	    if(comboSobj.id=="zddh"){
	       if($id("cNf") != null){
		      comboSobj.addParam("cNf",$id("cNf").value);
		   }
		   if($id("fxs") != null && $id("fxs").getSelectEntity() != null && $id("fxs").getSelectEntity().getProperty("vJgid") != null && $id("fxs").getSelectEntity().getProperty("vJgid") != ""){
		      comboSobj.addParam("vJgid",$id("fxs").getSelectEntity().getProperty("vJgid"));
		   }
	    }
