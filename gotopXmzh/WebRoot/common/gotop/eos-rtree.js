...
function _rteemodel_getDataset(node,checkedList,unCheckList,allClockList)
{
	if(node!=null){
		...
		if(entity.checked=='1' || entity.checked=='2'){
			if(checkedList){
				checkedList.push(entity);
			}
		}else if(entity.checked=='3'){
			if(allClockList){
				allClockList.push(entity);
			}
		}else{
			if(unCheckList){
				unCheckList.push(entity);
			}
		}
...
function _rtree_CheckedList(opType)
{
	...
	if(opType==1 || opType == 3 || opType == 5){
		checkedList=new CheckList("checked")
	}
	if(opType==2 || opType == 3 || opType == 6){
		unCheckList=new CheckList("unCheck")
	}
	if(opType==4 || opType == 5 || opType == 6){
		allClockList=new CheckList("allClock");
	}
...
function _rteemodel_add_infoList(infoName){
	...
	if(infoObj.entitySplitName){
		groupList.entitySplitName=(infoObj.entitySplitName).split(",");
	}
