...
function _rtreemodel_loadNodeChild( rtreeNode,jsHandle )
{
    ...
    this.processCheckBoxEntity(entities);
//根据节点的状态,设置节点的图标
	if(!rtreeNode.isroot)
	{
...
//    }
    ...
    var entitys=dataset.values;
    this.processCheckBoxEntity(entitys);
	...
	if(image!=""&&image!="null")
	 ...
	 if(level==""&&!hasRoot&&isFirstNode)
       ...
       //处理结点的连线
		...
		if((node.hasCheckbox && (showCheckType=="" || showCheckType==null || showCheckType==undefined))
			|| (showCheckType && node.hasCheckbox && showCheckType.indexOf(","+entity.name+",")>-1)){
	...
	if(this.entity.checked!="3"){
		...
		if(checkType=="simple"){
			if(this.entity.checked=="1"){
				...
				var Infol = this.getTree().model.checkedInfoList[entity.name];
				var selID=Infol.infoObj.getAttribute("relatedSelID");
				var pstr = entity.getProperty(Infol.infoObj.getAttribute("displayProperty"));
				var entityId= entity.getProperty(Infol.infoObj.getAttribute("entityProperty"));
				selID = window.parent.$id(selID);
				for (var ks = selID.options.length-1; ks >= 0; ks--) {
					var option = selID.options[ks];
					if(option.value==entityId){
						alert(pstr+" 已为被选,操作失败");
						return;
					}
				}				
...
function _rtree_checkAllChild(node)
{
				...
				var Infol = node.getTree().model.checkedInfoList[entity.name];
				if(Infol){}else{
					return;
				}
				var selID=Infol.infoObj.getAttribute("relatedSelID");
...
function _rteemodel_processCheckBox_Entity(entitys){
	for(var i=0;i<entitys.length;i++){
		var info = this.checkedInfoList[entitys[i].name];
		if(info){
			var selObj = window.parent.$id(info.infoObj.getAttribute("relatedSelID"));
			var est = info.entitySplit;
			var esArra = [];
			for(var field in info){
				if(field.startWith("entitySplit")){
					if(!isBlank(info[field])){
						esArra.push(info[field]);
					}
				}
			}
			var value = entitys[i].getProperty(info.property);
			if(selObj){
				for (var j = selObj.options.length-1; j >= 0; j--) {
					var ops = selObj.options[j];
					if(ops.treeParentNode){
						continue;
					}
					if(ops.treeNode){}else{
						continue;
					}
					var estValue = ops.treeNode.entity.getProperty(info.property);
					if(value==estValue){
						entitys[i].checked="1";
						entitys[i].selOptions = ops;
						break;
					}
				}
			}
		}
		var info = this.clockList[entitys[i].name];
		if(info){
			var est = info.entitySplit;
//			var estName = info.entitySplitName;
			var esArra = [];
			for(var field in info){
				if(field.startWith("entitySplit")){
					esArra.push(info[field]);
			...
			for(var j=0;j<est.length;j++){
				if(value==est[j]){
					for(var k=0;k<esArra.length;k++){
						if(esArra[k].length>0 && esArra[k][j]){
							esArra[k].splice(j,1);
						}
					}
					entitys[i].checked=3;
...
function _rteemodel_add_infoList(infoName){
	var infoObj = window.parent.$id(infoName) || window.parent.$name(infoName);
	var groupList = new Object();
	groupList.property=infoObj.getAttribute("entityProperty");
	groupList.displayProperty=infoObj.getAttribute("displayProperty");
...
//	_rteenode_init_addOptions(infoName);
	...
	this.checkedInfoList[infoObj.getAttribute("entityName")]=groupList;
//	alert(infoObj.getAttribute("entityName")+"--------entity.name-"+this.checkedInfoList[infoObj.getAttribute("entityName")]);
}
...
function _rteemodel_add_clockList(infoName){
	...
	var groupList = new Object();
	groupList.property=infoObj.getAttribute("entityProperty");
	groupList.entitySplit=(infoObj.getAttribute("entitySplit")).split(",");
	this.clockList[infoObj.getAttribute("entityName")]=groupList;
}
...
function relatedSelByNode(parentNode,node,entity,isFirstNode,isLastNode,hasRoot,iconsrc,iconType,name){
	var Infol = parentNode.getTree().model.checkedInfoList[entity.name];
	var selID = null;
	if(Infol && Infol.infoObj && Infol.infoObj.getAttribute("relatedSelID")){
		selID=Infol.infoObj.getAttribute("relatedSelID");
	}
	selID = window.parent.$id(selID);
	if(selID){
		...
		if(entity.checked){
			...
			var dop = Infol.infoObj.getAttribute("displayOtherProperty");
			var titleStr = "";
			if(dop){
			...
			oOption.value = entity.getProperty(Infol.infoObj.getAttribute("entityProperty"));
		...
		var checkcount = Infol.infoObj.getAttribute("checkcount");
		if(checkcount){
			if(checkcount<selID.options.length){
				window.parent.deleteOption(selID,0)
			}
		}
	}
}
...
function _rteenode_init_addOptions(butID){
		...
		var dialogArguments = window.parent.window["dialogArguments"];
		var peArgument = dialogArguments[mngemps.entityName];
		if(isBlank(peArgument)){
			return;
		}
		var arraEntity = (peArgument[0]).arraEntity;
		var selObj = window.parent.$id(mngemps.getAttribute("relatedSelID"));
		if(isBlank(selObj)){
			return;
		}
		if(peArgument[1]){
			mngemps.setAttribute("displayProperty",peArgument[1]);
		}
		var args = Array.prototype.slice.call(peArgument,2);//截取从第三个参数开始的所有参数
		if(args.toString()){
			mngemps.setAttribute("displayOtherProperty",args.toString());
			mngemps.setAttribute("entityProperty",args.toString());
		}
		for(var i=0;i<arraEntity.length;i++){
			var entity = arraEntity[i];
			var pstr = entity.getProperty(mngemps.getAttribute("displayProperty"));
			if(isBlank(pstr)){
				continue;
			}
			oOption = document.createElement("OPTION");
			...
			var dop = mngemps.getAttribute("displayOtherProperty");
			if(dop){
				var dopara = dop.split(",");
				for(var k=dopara.length-1;k>=0;k--){
					if(dopara[k]){
			...
			oOption.innerText = pstr;
			oOption.value = entity.getProperty(mngemps.getAttribute("entityProperty"));
			oOption.treeNode={};	
