	 ...
	 if(level==""&&!hasRoot&&isFirstNode)
       ...
       //处理结点的连线
		...
		if((node.hasCheckbox && (showCheckType=="" || showCheckType==null || showCheckType==undefined))
			|| (showCheckType && node.hasCheckbox && showCheckType.indexOf(","+entity.getProperty(showCheckEntity)+",")>-1)){
	...
	if(this.entity.checked!="3"){
		...
		if(checkType=="simple"){
			if(this.entity.checked=="1"){
				...
				var selID=Infol.infoObj.relatedSelID;
				var pstr = entity.getProperty(Infol.infoObj.displayProperty);
				var entityId= entity.getProperty(Infol.infoObj.entityProperty);
				selID = $id(selID);
/**
...
function _rteemodel_processCheckBox_Entity(entitys){
	for(var i=0;i<entitys.length;i++){
		...
		if(info){
			var selObj = $id(info.infoObj.relatedSelID);
...
function relatedSelByNode(parentNode,node,entity,isFirstNode,isLastNode,hasRoot,iconsrc,iconType,name){
	...
	var selID = null;
	if(Infol && Infol.infoObj && Infol.infoObj.relatedSelID){
		selID=Infol.infoObj.relatedSelID;
	}
	selID = $id(selID);
	if(selID){
		...
		if(entity.checked){
			...
			var dop = Infol.infoObj.displayOtherProperty;
			var titleStr = "";
			if(dop){
...
//			}
			...
			oOption.value = entity.getProperty(Infol.infoObj.entityProperty);
		...
		var checkcount = Infol.infoObj.checkcount;
		if(checkcount){
