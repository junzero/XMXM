//----- RTree ---------------
function RTree( id, width, height,hasRoot,relatedSel,checkType,checkButton)
{
	var treeModel = new Object();
	treeModel.hasRoot=hasRoot;
	treeModel.relatedSel = relatedSel;
	treeModel.checkType = checkType;
	treeModel.checkButton = checkButton;
	treeModel.aliasId = id.id;
    ...
    treeModel.processCheckBoxEntity = _rteemodel_processCheckBox_Entity;
    treeModel.beforeProcessCheckBox = _rteemodel_beforeProcessCheckBox;
    treeModel.initInfoList          = _rteemodel_init_infoList;
    treeModel.addInfoList           = _rteemodel_add_infoList;
    treeModel.initClockList         = _rteemodel_init_clockList;
    treeModel.addClockList           = _rteemodel_add_clockList;
	//属性 
	...
	treeModel.datasetList=new Object();
	treeModel.datasetList.root=new Object();
	treeModel.checkedInfoList=new Object();
	treeModel.afterChecked= _rteemodel_afterChangeChecked;
	treeModel.beforeChecked= _rteemodel_beforeChangeChecked;
	treeModel.clockList=new Object();
...
function _rtreemodel_loadNodeChild( rtreeNode,jsHandle )
{
	...
	if(rtreeNode.isloadData!==true)
	{
		...
		rtreeNode.getTree().model.datasetList[rtreeNode.entity.name]=dataset;
    ...
    var model = rtreeNode.getTree().model;
    this.processCheckBoxEntity(entities,model);
//根据节点的状态,设置节点的图标
	if(!rtreeNode.isroot)
	{
...
//    for(var k=0;k<dataset.getLength();k++){
//    	alert(rtreeNode.entity.name+"\n"+entitys[k]);
//    }

    var entitys=dataset.values;
    var model = rtreeNode.getTree().model;
    this.processCheckBoxEntity(entitys,model);
    
	model.datasetList[rtreeNode.entity.name]=dataset;
...
function _rtreemodel_expandRoot()
{
		...
		if(this.checkType){
			rootNode.checkType=window.checkType;
		}
	    rootNode.level = 0;
...
function _rtreemodel_setEntityInfo(entityName, exp, iconSrc, onclick, onrefresh, ondblclick,iconType,InitExpandParamFunc,childEntityXpaths,expandAction,submitXpath,preload,
	fieldCheck,entityProperty,displayProperty,displayOtherProperty,checkCount,hasCheck)
{
	entityName = entityName.toLowerCase();
	var entityInfo = this.getEntityInfo( entityName );
	...
	entityInfo.hasCheck=hasCheck;
	entityInfo.fieldCheck=fieldCheck;
	entityInfo.entityProperty=entityProperty;
	entityInfo.displayProperty=displayProperty;
	entityInfo.displayOtherProperty=displayOtherProperty;
	entityInfo.checkCount=checkCount;
}
...
function _rtreemodel_showNodeMenu( rtreeNode )
{
	...
	if( menuItems.length > 0 )
	{
		...
		this.menu.show();
		try{
			var zIndex = $id(window["_eos_dialog_id"]+"__model_dialog_container").style.zIndex;
...
function RTreeView( id, model,  width, height )
{
    ...
    */
    ...
    treeview.checkLocked       = _rteeview_associate_locked;
    treeview.beforeInit            =_rteemodel_before_init;
    
    treeview.getDataset            = _rteemodel_getDataset;
    treeview.getCheckedList = _rtree_CheckedList;
...
function RTreeNode( parentNode,name, iconsrc, entity ,isFirstNode,isLastNode,hasChild,hasRoot,iconType)
{
	   ...
	   // node.style.border="1px solid black"
	   	node.hasCheckbox=parentNode.hasCheckbox;
		node.checkType=parentNode.checkType;
	    node.openIconSrc=STREE_DEFAULTOPEN_ICON;
	    node.closeIconSrc=STREE_DEFAULTCLOSE_ICON;
	    node.plusIconSrc="";
	    node.minusIconSrc="";

		var rtreeView = findRTree(parentNode);
		var model = rtreeView.model;
	    node.entityInfo=model.getEntityInfo(entity.name);
	    ...
	    node.changeCheckboxState = _rteenode_op_changeCheckboxState;
		
//		relatedSelByNode(parentNode,node,entity,isFirstNode,isLastNode,hasRoot,iconsrc,iconType,name);
	    _rtreeNode_processNodeStatus(node,entity,isFirstNode,isLastNode,hasRoot,iconsrc,iconType,name,model)
	/*
	...
	*/
	...
	node.checkbox_click = _rtreeNode_checkbox_onchick;
  ...
  node.setLeaf            =_rtreeNode_setLeaf;
	...
	node.isChecked      = _rtreeNode_isChecked;
	//node.onmouseup		= _rtreenode_onmouseup;
	if(node.entityInfo){
		node.entityInfo.oncheck      = model.onCheckFunc;
	}
	if(model.beforeChecke){
		node.beforeChecke = model.beforeChecke;
	}
	if(model.afterChecke){
		node.afterChecke = model.afterChecke;
	}
	return node;
}
...
function _rtreeNode_processNodeStatus(node,entity,isFirstNode,isLastNode,hasRoot,icon,iconType,name,model){
	var level=node.imgLevel;
	var image=icon;
	var imageType=iconType;
	
	var showCheckType = model.showCheckType;
	var showCheckEntity = model.showCheckEntity;
	...
	if(image!=""&&image!="null")
	 ...
	 if(level==""&&!hasRoot&&isFirstNode)
       ...
       //处理结点的连线
        ...
        var hasCheckbox = model.getEntityInfo(entity.name).hasCheck;
		if(hasCheckbox){
		    ...
		    node_content.appendChild(checkbox);
		}
...
//		changeButionTree(rtreeView.model,this);
	}
	/**
	* 锁定与解锁
	**/
	function changeButionTree(model,node){
		var checkBut = $id(model.checkButton) || $name(model.checkButton);
		if(checkBut==null){
			return null;
		}
...
function _rtreeNode_expandNode(jsHandle)
{
	...
	if(!rtreeNode.childLoaded ){
			...
			oldSetTimeout(function(){return _rtreemodel_loadNodeChild.apply(model,[rtreeNode,jsHandle])},1);
...
/*	
	if(this.entity.checked=="3"){
		this.cell.innerHTML = this.cell.innerHTML+"(级联锁定)";
	}
*/
 ...
 * 处理点击checkbox
 */
function _rtreeNode_checkbox_onchick()
{
	if(this.beforeChecke && this.beforeChecke(this)==false){return;}
	if(this.entity.checked!="3"){
		var checkType = findRTree(this).model.checkButton;
		if(checkType){
			if($id(checkType)){
				checkType = $id(checkType).checkType;
			}else{
				checkType="simple";
			}
		}else{
			checkType = findRTree(this).model.checkType;
		}
		
		if(checkType=="simple"){
			if(!this.getTree().model.beforeChecked(this)){
				return false;
			}
			...
			if(!this.getTree().model.afterChecked(this)){
				return false;
			}
//			relatedSelByNode(this.getParent(),this,this.entity);
	   	}else{
			if(this.entity.checked=="1"){
				_rtree_uncheckAllChild(this)
			}else{
				_rtree_checkAllChild(this)
			}
	//	_rtree_process_parentNode(this);
		}
	}
   ...
   if(this.afterChecke && this.afterChecke(this)==false){return;}
}

function _rtree_uncheckAllChild(node)
{
	if(!node.getTree().model.beforeChecked(node)){
		return false;
	}
	node.entity.checked=0;
	node.checkbox.src=STREE_CHECKBOX_FALSE_ICON;
	if(!node.getTree().model.afterChecked(node)){
		return false;
	}
  ...
  if(node.hasChild)
    if(node.childLoaded)
	{
		...
		for(var i=0;i<childs.length;i++)
		   _rtree_uncheckAllChild(childs[i]);
	}
	else
	{
        var childs=node.getChildren();
		for(var i=0;i<childs.length;i++)
		   _rtree_unload_uncheckAllChild(childs[i]);
	}
//	relatedSelByNode(node.getParent(),node,node.entity);
	return;
}
function _rtree_unload_uncheckAllChild(node)
{
	if(!node.getTree().model.beforeChecked(node)){
		return false;
	}
...
function _rtree_checkAllChild(node)
{
			if(!node.getTree().model.beforeChecked(node)){
				return false;
			}
			  ...
			  if(node.hasChild){
			    if(node.childLoaded){
					var childs=node.getChildren();
					for(var i=0;i<childs.length;i++)
					   _rtree_checkAllChild(childs[i]);
				}
				else{
			        var childs=node.getChildren();
					for(var i=0;i<childs.length;i++)
					   _rtree_unload_checkAllChild(childs[i]);
				}
			  }
//  relatedSelByNode(node.getParent(),node,node.entity);
  return;
...
function _rtree_unload_checkAllChild(node)
{
	node.entity.checked=0;
    if(node.getChildren)
	{
 ...
 * 得到树实体
**/
function _rteemodel_getDataset(node,checkedList,unCheckList,allClockList)
{
	if(node!=null){
		var entity = node.getEntity();
		if(entity.checked=='1' || entity.checked=='2'){
			if(checkedList){
				if(isBlank(checkedList[entity.name])){
					checkedList[entity.name]=[];
				}
				checkedList[entity.name].push(entity);
			}
		}else if(entity.checked=='3'){
			if(allClockList){
				if(isBlank(allClockList[entity.name])){
					allClockList[entity.name]=[];
				}
				allClockList[entity.name].push(entity);
			}
		}else{
			if(unCheckList){
				if(isBlank(unCheckList[entity.name])){
					unCheckList[entity.name] = [];
				}
				unCheckList[entity.name].push(entity);
			}
		}
		if(node.hasChildNode()){
			var child = node.getChildren();
			for(var i=0;i<child.length;i++){
				_rteemodel_getDataset(child[i],checkedList,unCheckList,allClockList);
			}
		}
	}
}
 ...
 * opType=1 checked
 * opType=2 unCheck
 * opType=1+2 checked + unCheck
 * opType=4 allCheck
 * opType=1+4 checked+allCheck
 * opType=2+4 unCheck+allCheck
 */
function _rtree_CheckedList(opType)
{
	var checkedList=null;
	var unCheckList=null;
	var allClockList=null;
	if(opType==1 || opType == 3 || opType == 5){
		checkedList={};
	}
	if(opType==2 || opType == 3 || opType == 6){
		unCheckList={};
	}
	...
	var checkList =new Object();
		...
		for(var i=0;i<children.length;i++){
			this.getDataset(children[i],checkedList,unCheckList,allClockList);
		}
	checkList["checked"]=checkedList;
	checkList["unCheck"]=unCheckList;
	checkList["allClock"]=allClockList;
		
	return checkList;
}
function _rteemodel_processCheckBox_Entity(entitys,model){
	for(var i=0;i<entitys.length;i++){
		model.beforeProcessCheckBox(entitys[i]);
	}
}

function _rteeview_associate_locked(button){
	if(button){}else{
		button=$id(this.model.checkButton) || $name(this.model.checkButton);
	}
	var sn = this.getSelectNode();
	if(sn.entity.checked==3){
		sn.entity.checked=0;
/*		
		var text = sn.getText();
		var ssIndex = text.lastIndexOf("(级联锁定)");
		var ttstr = text.substr(0,ssIndex);
		sn.setText(ttstr);
		button.value="级联锁定";
*/
		sn.checkbox.src=STREE_CHECKBOX_FALSE_ICON;
	}else{
		sn.entity.checked=3;
/*
		var text = sn.getText();
...
function _rteemodel_init_infoList(infoList){
	if(infoList){
		if(infoList.splice){
			for(var i=0;i<infoList.length;i++){
				this.addInfoList(infoList[i]);
			}
		}else{
...
function _rteemodel_add_infoList(infoName){
	var infoObj = $id(infoName) || $name(infoName);
	var groupList = new Object();
	groupList.property=infoObj.entityProperty;
	groupList.displayProperty=infoObj.displayProperty;
	for(var field in infoObj){
		if(field.startWith("entitySplit")){
			var entitySplit = infoObj[field];
			if(entitySplit){
				groupList[field]=(entitySplit).split(",");
			}else{
				groupList[field]=[];
			}
		}
	}
//	_rteenode_init_addOptions(infoName);
	if(isBlank(this.infoObjArra)){
		this.infoObjArra=[];
	}
	this.infoObjArra.push(infoObj);
	groupList.infoObj=infoObj;
	this.checkedInfoList[infoObj.entityName]=groupList;
}

function _rteemodel_init_clockList(clockList){
	if(clockList){
		if(clockList.splice){
			for(var i=0;i<clockList.length;i++){
				this.addClockList(clockList[i]);
			}
		}else{
			this.addClockList(clockList);
		}
	}
}
function _rteemodel_add_clockList(infoName){
	var infoObj = $id(infoName) || $name(infoName);
	var groupList = new Object();
	groupList.property=infoObj.entityProperty;
	groupList.entitySplit=(infoObj.entitySplit).split(",");
	this.clockList[infoObj.entityName]=groupList;
}
function _rteemodel_before_init(model){
	model.hasCheckbox=true;
	model.checkType="associated";
}

function relatedSelByNode(parentNode,node,entity,isFirstNode,isLastNode,hasRoot,iconsrc,iconType,name){
	var model = parentNode.getTree().model;
	var entityInfo = model.getEntityInfo(entity.name);
	var selID = $id(model.relatedSel);
	if(selID){
		var oOption = node.optionSel;
		if(entity.checked){
			var pstr = entity.getProperty(entityInfo.displayProperty);
			if(oOption){}else{
				oOption = entity.selOptions;
				if(oOption){}else{
					oOption = document.createElement("OPTION");
					oOption.selType=entity.name;
					selID.options.add(oOption);
				}
			}
			entity.selOptions=null;
			var dop = entityInfo.displayOtherProperty;
			var titleStr = "";
			if(dop){
				var dopara = dop.split(",");
				for(var k=dopara.length-1;k>=0;k--){
					if(dopara[k]){
						dopara[k]=entity.getProperty(dopara[k]);
					}
				}
				titleStr += "("+dopara.join(",")+")";
			}
			if(setRelatedNodeText){
				pstr += setRelatedNodeText(entity);
			}
//			var name="";
//			if(parentNode.getProperty){
//				name = "/"+parentNode.getText();
//			}
			oOption.innerText = pstr+titleStr;
			oOption.title = titleStr;
			oOption.value = entity.getProperty(entityInfo.entityProperty);
			oOption.treeNode=node;
			oOption.treeParentNode=parentNode;
			node.optionSel=oOption;
		}else if(oOption && selID){
			selID.removeChild(oOption);
			node.optionSel=null;
		}
		var checkcount = entityInfo.checkCount;
		if(checkcount){
			if(checkcount<selID.options.length){
				deleteOption(selID,0)
			}
		}
	}
}
function _rteenode_op_changeCheckboxState(){
	var model = this.getTree().model;
	model.beforeProcessCheckBox(this.entity);
...
//已删除
function _rteenode_init_addOptions(butID){
		var mngemps = $id(butID);
		var dialogArguments = window["dialogArguments"];
		var peArgument = dialogArguments[mngemps.entityName];
		if(isBlank(peArgument)){
			return;
		}
		var arraEntity = (peArgument[0]).arraEntity;
		var selObj = $id(mngemps.relatedSelID);
		if(isBlank(selObj)){
			return;
		}
		if(peArgument[1]){
			mngemps.displayProperty=peArgument[1];
		}
		var args = Array.prototype.slice.call(peArgument,2);//截取从第三个参数开始的所有参数
		if(args.toString()){
			mngemps.displayOtherProperty=args.toString();
			mngemps.entityProperty=args.toString();
		}
		for(var i=0;i<arraEntity.length;i++){
			var entity = arraEntity[i];
			var pstr = entity.getProperty(mngemps.displayProperty);
			if(isBlank(pstr)){
				continue;
			}
			oOption = document.createElement("OPTION");
			...
			var dop = mngemps.displayOtherProperty;
			var titleStr = "";
			if(dop){
				var dopara = dop.split(",");
				for(var k=dopara.length-1;k>=0;k--){
			...
			oOption.innerText = pstr+titleStr;
			oOption.value = entity.getProperty(mngemps.entityProperty);
			oOption.treeNode={};	
			oOption.treeNode.entity=entity;	
			oOption.treeNode.changeCheckboxState=function(){};	
		}
}
function _rteemodel_afterChangeChecked(node){
	var selID = $id(this.relatedSel);
	if(selID){}else{
		return;
	}
	var entity = node.entity;
	var entityInfo = this.getEntityInfo(entity.name);
	var fieldCheck = entityInfo.fieldCheck;
	if(fieldCheck!=null && entity.getProperty(fieldCheck)){
		if(entity.checked){
			for(var k = 0;k<selID.options.length;k++){
				var sop = selID.options[k];
				if(node==sop.treeNode){
					selID.removeChild(sop);
				}
			}
			return;
		}
	}else{
		if(entity.checked){}else{
			for(var k = 0;k<selID.options.length;k++){
				var sop = selID.options[k];
				if(node==sop.treeNode){
					selID.removeChild(sop);
	...
	oOption.selType=entity.name;
	oOption.entity = entity;
	oOption.opType = entity.checked;
	oOption.treeNode = node;
	var entityProperty = entityInfo.entityProperty;
	if(entityProperty){
		var dopara = entityProperty.split(";");
		for(var k=dopara.length-1;k>=0;k--){
			if(dopara[k]){
				dopara[k]=entity.getProperty(dopara[k]);
			}
		}
		entityProperty = dopara.join("_");
	}
	oOption.value = entityProperty;
	for(var k = 0;k<selID.options.length;k++){
		var option = selID.options[k];
		if(option.value ==entityProperty){
			selID.removeChild(option);
		}
	}
	selID.options.add(oOption);
	
	var displayProperty = entityInfo.displayProperty;
	if(displayProperty){
		var dopara = displayProperty.split(";");
		for(var k=0;k<dopara.length;k++){
			if(dopara[k]){
				dopara[k]=entity.getProperty(dopara[k]);
			}
		}
		displayProperty = dopara.join("/");
	}
	if(entity.checked){
		oOption.innerText = displayProperty+"(新增)";
	}else{
		oOption.innerText = displayProperty+"(删除)";
	}
	var displayOtherProperty = entityInfo.displayOtherProperty;
	if(displayOtherProperty){
		var dopara = displayOtherProperty.split(";");
		for(var k=dopara.length-1;k>=0;k--){
			if(dopara[k]){
				dopara[k]=entity.getProperty(dopara[k]);
			}
		}
		oOption.title = "("+dopara.join(",")+")";
	}
	return true;
}
function _rteemodel_beforeChangeChecked(node){
//	alert(node.entity);
	return true;
}
function _rteemodel_beforeProcessCheckBox(entity){
	var fieldCheck = this.getEntityInfo(entity.name).fieldCheck;
	if(fieldCheck!=null && entity.getProperty(fieldCheck)){
		entity.checked="1";
	}
	return true;
}
var setRelatedNodeText=null;
