...
function fDrawCal(iYear, iMonth, iCellHeight, iDateTextSize) {
  ...
  var styleTD = " bgcolor='"+gcBG+"' bordercolor='"+gcBG+"' valign='middle' align='center' height='"+iCellHeight+"' style='font:bold "+iDateTextSize+" 宋体;";
	...
	write("<tr>");
	for(i=0; i<7; i++)
		write("<td "+styleTD+" color:#406AC0' >" + WeekDay[i] + "</td>");
	write("</tr>");
  	for (w = 1; w < 7; w++) {
		write("<tr>");
		for (d = 0; d < 7; d++) {
...
function fToggleTags(){
  var seobj = document.getElementsByTagName("SELECT");
  with (seobj){
 	for (i=0; i<length; i++)
...
write("<table border='0' bgcolor='#FAFAFA'>");
write("<TR>");
write("<td valign='middle' align='center'><input type='button' name='PrevMonth' value='<' style='background:#F1F0F8;height:20;width:20;FONT:bold' onClick='fPrevMonth()'>");
...
write("</SELECT>");
write("&nbsp;<input type='button' name='PrevMonth' value='>' style='background:#F1F0F8;height:20;width:20;FONT:bold' onclick='fNextMonth()'>");
write("</td>");
write("</TR><TR>");
...
write("<B style='cursor:hand;font:bold 12 宋体' onclick='fSetDate(giYear,giMonth,giDay)' onMouseOver='this.style.color=gcDay' onMouseOut='this.style.color=0'>今天："+giYear+"年"+giMonth+"月"+giDay+"日</B>  <a href='javascript:fHideCalendar()' >关闭</a>");
write("</TD></TR>");
write("</TABLE></Div>");
}
