...
<script>
    ...
    function addRecord() {
        ...
        var url = "org.gocom.abframe.org.workgroup.GroupOrgMaintain.flow";
        url += "?_eosFlowAction=addOrg";
        url += "&groupID=";
        url += '<b:write property="groupID" scope="flow"/>';
        url += '&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
	    showModalCenter(url, "", callBack, 550, 400, "增加工作组机构"); <!--增加工作组机构 -->
    }
    //删除工作组员工
    function delRecord() {
		if(window.parent.$name("group/empid").value!=$name("userIntegrated/empid").value){
...
<h:form name="queryForm" action="org.gocom.abframe.org.workgroup.GroupOrgMaintain.flow" checkType="blur" target="_self" method="post" onsubmit="return checkForm(this);">
  <input type="hidden" name="_eosFlowAction" value="pageQuery"/>
  <h:hidden name="criteria/_entity" value="org.gocom.abframe.org.publicom.publicom.VGroupMemberByOrg"/>
  <h:hidden name="criteria/_expr[1]/groupid" property="groupID" scope="flow"/>
  <w:panel id="panel1" width="100%" title="工作机构工查询"> <!--工作机构工查询 -->
    <table align="center" border="0" width="100%" class="form_table">
      <tr>
        ...
        <td colspan="1">
          <h:hidden name="groupid" property="groupID" scope="flow"/>
          <h:text property="criteria/_expr[2]/orgcode"/>
        ...
        <td colspan="1">
          <h:text property="criteria/_expr[3]/orgname"/>
...
<h:form name="viewlist1" action="org.gocom.abframe.org.workgroup.GroupOrgMaintain.flow" checkType="blur" target="_self" method="post">
  <input type="hidden" name="_eosFlowAction" value="pageQuery" >
  ...
  <h:hiddendata property="criteria"/>
    <table align="center" border="0" width="100%" class="EOS_table">
         <tr>
	       <td colspan="4" class="eos-panel-title">&nbsp;机构工列表</td>
	      </tr>
          ...
          <tr class="<l:output evenOutput='EOS_table_row' oddOutput='EOS_table_row_o' />">
            <td align="center">
              <w:rowCheckbox afterSelectFunc="clickCheck($id('group1'), $id('updateButton'), $id('deleteButton'))" afterUnSelectFunc="clickCheck($id('group1'), $id('updateButton'), $id('deleteButton'))">
                ...
                <h:param name='selectedObjects[*]/memberid' iterateId='id1' property='orgid' indexed='true' />
                <h:param name='selectedObjects[*]/flag' iterateId='id1' property='flag' indexed='true' />
              </w:rowCheckbox>
            </td>
                <td>
                ...
                <td>
                  <d:write iterateId="id1" property="orgdegree" dictTypeId="ABF_ORGTYPE"/>
                </td>
          </tr>
        </l:iterate>
      </w:checkGroup>
