...
<script>
	...
	function queryGroup(node) {
		...
		parent.window.frames["groupTabs"].location.href="/workgroup/groupmanagerAction_queryGroupViewList.action";
	}
	//显示GroupTabs页面
	function displayGroupTabs(node) {
		...
		parent.window.frames["groupTabs"].location.href="/workgroup/groupmanagerAction_toTab.action?"+params;
	}
	...
	function addGroup(node){
	    ...
	    url += "?isAdd=true";
	    var gtype = node.entity.getProperty("grouptype");
	    if(gtype){
	    	url += "&grouptype="+gtype;
	    }else{
		    url += "&grouptype="+node.entity.getProperty("dictID");
	    }
	    var parentgroupid = node.getProperty("groupid");
	    if(gtype && parentgroupid){
	    	title="<b:message key='workgroupManager_l_addSubGroup' />";
	     	url += "&group.parentgroupid="+parentgroupid;
	    }else{
	...
	function addTopGroup(node) {
	     ...
	     url += '?_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
	     showModalCenter(url, "", refreshSelectedNode, 470, 230, "新增群姐");  <!--新增工作组 -->
	}
   ...
   */
    ...
    */   
    function custInit(){  
    	//树高度调整
        var ht = document.body.clientHeight-TREE_MODIFY_HEIGHT-10;
		$id("groupTree_container").style.height =ht;
...
<body leftmargin="0" topmargin="0"><h:hidden property="userIntegrated/empid" scope="session" />
	...
	<r:rtree id="groupTree" width="100%" height="100%">
	  <r:treeRoot action="/workgroup/groupmanagerAction_queryRootGroupTypeDatacell.action" onDblclickFunc="dbClickNode"
	  ...
	  <r:treeNode action="/workgroup/groupmanagerAction_queryRootGroupTree.action" childEntities="groupList"
	  ...
	  <r:treeNode action="/workgroup/groupmanagerAction_queryGroupPositionEmpByParentTree.action" onDblclickFunc="dbClickNode"
	  ...
	  <r:treeNode nodeType="positionList" onDblclickFunc="dbClickNode" childEntities="empOfPositionList" onClickFunc="displayPositionTabs" 
	  	showField="posiname" icon="/common/images/icons/group.png" submitXpath="position">
...
<%--	    <r:treeMove toNode="groupList" initParamFunc="getInitParam" action="org.gocom.abframe.org.workgroup.WorkgroupManager.moveOrCopyEmpFromGroupToGroup.biz"/>--%>
	  ...
	  <r:treeNode nodeType="roleList" showField="rolename" icon="/common/images/icons/arrow_merge.gif" onRefreshFunc="roleListRefresh" submitXpath="role">
<%--	    <r:treeMove toNode="groupList" initParamFunc="getInitParam" action="org.gocom.abframe.org.workgroup.WorkgroupManager.moveOrCopyEmpFromGroupToGroup.biz"/>--%>
	  </r:treeNode> 
	</r:rtree>
	</td>
